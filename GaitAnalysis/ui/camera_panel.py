...
class CameraPanel(ctk.CTkFrame):
    ...
    def _build_ui(self):
        ...
        self._module_var = ctk.StringVar(value="webcam")
        self._module_combo = ctk.CTkComboBox(
            main_frame, values=["webcam", "depth"],
        ...
        row += 1
        self._status_label = ctk.CTkLabel(main_frame, text=Config.lang("camera.placeholder", ""), font=ctk.CTkFont(size=12))
    ...
    def _on_module_change(self, event=None):
        ...
        self._controller.camera_module = module
    ...
    def _toggle_camera(self):
        if self._controller.state_machine.state.name in ("CAMERA_READY", "RECORDING"):
            ...
            try:
                ...
                h = 170.0
            self._controller.camera_module = self._module_var.get()
            ok = self._controller.start_camera()
            if ok:
                self._camera_btn.configure(text=Config.lang("camera.stop", "Stop Camera"))
                self._status_label.configure(text=f"{Config.lang('camera.camera_type', 'Camera')}: Webcam")
            else:
